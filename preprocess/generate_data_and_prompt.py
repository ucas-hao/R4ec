...
def generate_train_ctr_data(sequence_data, train_test_split):
    # print(list(lm_hist_idx.values())[:10])
    full_data = []
    total_label = []
    for uid in sequence_data:
        item_seq, rating_seq = sequence_data[str(uid)]
        for idx in range(11,train_test_split[str(uid)]+1):
            label = 1 if rating_seq[idx] > rating_threshold else 0
            full_data.append([uid,idx,label])
            total_label.append(label)
    print('data num', len(full_data), 'pos ratio', sum(total_label) / len(total_label))
    return full_data
    # for example 这里的['1',11,1]到['1',116,0]
def generate_test_ctr_data(sequence_data, train_test_split):
    full_data = []
    total_label = []
    for uid in sequence_data:
        item_seq, rating_seq = sequence_data[str(uid)]
        for idx in range(train_test_split[str(uid)]+1,len(item_seq)):
            label = 1 if rating_seq[idx] > rating_threshold else 0
            full_data.append([uid,idx,label])
            total_label.append(label)
    print('data num', len(full_data), 'pos ratio', sum(total_label) / len(total_label))
...
if __name__ == '__main__':
    ...
    SPLIT_PATH = os.path.join(PROCESSED_DIR, 'train_test_split_index.json')
    ...
    train_set = load_json(Train_path)
    test_set = load_json(Test_path)
    
    print('final loading data')
    # print(list(item2attribute.keys())[:10])

    print('generating ctr train dataset')
    train_ctr = generate_train_ctr_data(sequence_data, train_test_split)
    print('generating ctr test dataset')
    test_ctr = generate_test_ctr_data(sequence_data, train_test_split)
    print('save ctr data')
...
# data num 737358 pos ratio 0.5716517621019912
# generating ctr test dataset
# data num 196411 pos ratio 0.5626772431279308
# save ctr data

# final loading data
# generating ctr train dataset
# data num 1144308 pos ratio 0.6032353177640984
# generating ctr test dataset
# data num 309898 pos ratio 0.5945343306507302
# save ctr data
