...
class BaseModel(nn.Module):
    ...
    def process_input(self, inp):
        ...
        hist_item_emb = self.item_embedding(inp['hist_iid_seq'].to(device)).view(-1, self.max_hist_len, self.embed_dim) # [256,10] [256,10,32]
        hist_attr_emb = self.attr_embedding(inp['hist_aid_seq'].to(device)).view(-1, self.max_hist_len, # [256,10,32*2]
                                                                                 self.embed_dim * self.attr_fnum) 
        hist_rating_emb = self.rating_embedding(inp['hist_rate_seq'].to(device)).view(-1, self.max_hist_len, # torch.Size([256, 10, 32])
                                                                                      self.embed_dim)
        hist_emb = torch.cat([hist_item_emb, hist_attr_emb, hist_rating_emb], dim=-1) # item knowledge + attribute knowledge + rating knowledge torch.Size([256, 10, 128])
        hist_len = inp['hist_seq_len'].to(device) 

        if self.task == 'ctr':
            iid_emb = self.item_embedding(inp['iid'].to(device)) # [256,32]
            ...
            if self.augment_num: # 这里为[torch.Size([256, 768]),torch.Size([256, 768])]即item和user的embeddings
                orig_dens_vec = [inp['hist_aug_vec'].to(device), inp['item_aug_vec'].to(device)]
                dens_vec = self.convert_module(orig_dens_vec) # [256,64]
            else:
                dens_vec, orig_dens_vec = None, None # item_embeddings有target item的所有信息，hist_emb有历史的item的所有信息，hist_len为历史的长度，dense_vec是orig_dense_vec转换后的embedding
...
class GRU4REC(BaseModel):
    def __init__(self, args, dataset):
        ...
        self.intra_item_attn = nn.MultiheadAttention(self.itm_emb_dim, 4, batch_first=True, dropout=0)
        self.intra_hist_gru = nn.GRU(self.hist_emb_dim, self.hidden_size, dropout=self.rnn_dp, batch_first=True)

        self.final_mlp = MLP(self.final_mlp_arch, self.module_inp_dim, self.dropout)
        self.final_fc = nn.Linear(self.final_mlp_arch[-1], 1)

    def get_input_dim(self):
        self.dens_vec_num = 64
        if not self.augment_num:
            self.dens_vec_num = 0
        return 192 + 192 + self.dens_vec_num 
    ...
    def forward(self, inp):
        ...
        user_behavior = torch.cat([user_behavior, cross_hist], dim=-1)
        item_embedding = torch.cat([item_embedding, cross_item], dim=-1)

        user_behavior = torch.mean(user_behavior, dim=1)

        if self.augment_num:
            # print(user_behavior.shape, query.shape, dens_vec.shape)
            concat_input = torch.cat([user_behavior, item_embedding, dens_vec], dim=-1)
        else:
            concat_input = torch.cat([user_behavior, item_embedding], dim=-1)
...
class GRU4REC_movie(BaseModel):
    def __init__(self, args, dataset):
        super(GRU4REC_movie, self).__init__(args, dataset)
        self.intra_item_attn = nn.MultiheadAttention(self.itm_emb_dim, 4, batch_first=True, dropout=0)
        self.intra_hist_gru = nn.GRU(self.hist_emb_dim, self.hidden_size, dropout=self.rnn_dp, batch_first=True)
    ...
    def get_input_dim(self):
        ...
        if not self.augment_num:
            self.dens_vec_num = 0
        return 192 + 96 + self.dens_vec_num 
